...
description: 扫描 Android 工程中所有 module 的 res/values*/strings.xml 文件，检测 CJK 康熙部首、兼容表意文字、全角字符等异常内容，生成 Markdown 汇总报告。适用于 Android 字符串资源质量检查场景。
---
...
扫描 Android 工程中所有 module 内 `res/values-zh-*/strings.xml` 文件，检测以下 CJK 相关异常字符：

- **康熙部首字符** (Kangxi Radicals: 0x2F00-0x2FDF)
- **CJK 兼容表意文字** (Compatibility Ideographs: 0xF900-0xFAFF)
- **全角字符** (Fullwidth Forms: 0xFF00-0xFFEF)

使用 Unicode NFKC 归一化对比检测，找出归一化后发生变化的字符。

## 触发条件

- 用户要求检测 Android 字符串资源中的 CJK 异常字符
- 用户要求扫描 strings.xml 中的康熙部首、兼容字、全角字符
- 用户提到"异形字符"、"CJK异常"、"全角字符"等关键词
...
脚本会自动：
1. 查找所有 `res/values-zh-*/strings.xml` 文件
...
1. 脚本会递归扫描所有子目录中的 `res/values-zh-*/strings.xml` 文件
